            ...
            try {
                ...
                return if (result?.body()?.status==1){
                    ...
                    for (i in 0 until userProfileArray.size) {
                        val user_profile = UserRegistrationModel(0, userProfileArray[i].user_id,"",userProfileArray[i].user_name,userProfileArray[i].email,userProfileArray[i].mobile_number,"",
                               "", userProfileArray[i].state_id, userProfileArray[i].district_id,"","","")
